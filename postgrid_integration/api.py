...
from postgrid_integration.custom_scripts.py.purchase_invoice import (
    validate_mandatory_fields,
)
from postgrid_integration.custom_scripts.py.sales_invoice import (
    validate_mandatory_fields as validate_mandatory_fields_for_letter,
)
from postgrid_integration.utils import (
    create_response_log,
    get_payload,
    get_payload_for_letter,
    get_pdf_link,
    send_request,
)
...
def create_postgrid_payment(name, retry=False, raise_throw=False):
    ...
    if doc.custom_postgrid_cheque_reference:
        if not raise_throw:
            frappe.throw(
                f'Postgrid Cheque Reference is already generated for this invoice {get_link_to_form("Purchase Invoice", doc.name)}'
            )
        raise Exception(
            f'Postgrid Cheque Reference is already generated for this invoice {get_link_to_form("Purchase Invoice", doc.name)}'
        )
    validate_mandatory_fields(
        doc=doc, throw=False if raise_throw else True, raise_throw=raise_throw
    )
    if bank_acc_doc := frappe.db.get_value(
        "Bank Account",
        {
            "is_company_account": 1,
            "company": doc.company,
            "postgrid_bank_account_id": ["!=", ""],
            "disabled": 0,
        },
        ["name", "account", "postgrid_bank_account_id"],
        as_dict=1,
    ):
        args = frappe._dict(
            {
                ...
                "payload": get_payload(
                    ...
                    doc.name,
                    doc.bill_no,
                    bank_acc_doc.postgrid_bank_account_id,
        ...
        if retry:
            args.update(
                {
                    "retry": frappe.db.get_list(
                        "Postgrid Log",
                        {"voucher_name": doc.name},
                        ["sum(no_of_retry) as retry"],
                    )[0]["retry"]
                    + 1
                }
            )
        ...
        args = frappe._dict(
            {
                ...
                "paid_from_account_currency": frappe.db.get_value(
                    "Account", bank_acc_doc.account, "account_currency"
                ),
...
def cheque_update(**args):
    try:
        ...
        data = args["data"]
        if payment_entry := frappe.get_all(
            "Payment Entry",
            {"custom_postgrid_cheque_reference": data.get("id")},
            pluck="name",
        ):
            frappe.db.set_value(
                "Payment Entry",
                payment_entry[0],
                "custom_postgrid_cheque_status",
                data.get("status"),
            )
            ...
            if data.get("status") == "cancelled":
                frappe.delete_doc(
                    "Payment Entry", payment_entry[0], ignore_permissions=True
                )
                if purchase_invoice := frappe.get_all(
                    "Purchase Invoice",
                    {"custom_postgrid_cheque_reference": data.get("id")},
                    pluck="name",
                ):
...
def letter_update(**args):
    try:
        data = args["data"]
        if sales_invoice := frappe.get_all(
            "Sales Invoice",
...
def create_postgrid_letter(name, retry=False, raise_throw=False):
    ...
    validate_mandatory_fields_for_letter(
        doc=doc, throw=False if raise_throw else True, raise_throw=raise_throw
    )
