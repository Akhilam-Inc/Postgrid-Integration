      ...
      frappe.db
        ...
        .then((res) => {
          ...
          if (parseInt(res.message.enable)) {
            if (
              cur_frm.fields_dict.items.grid.get_selected_children().length == 0
            ) {
            ...
            if (frm.doc.items) {
              frappe.call({
                method:
                  "postgrid_integration.postgrid_integration.doctype.bulk_letter_creation_tool.bulk_letter_creation_tool.process_bulk_letter",
                args: {
                  invoice_list:
              ...
              frappe.msgprint(
                "In Order to Send Postgrid Letter Fetch the Invoices."
              );
          } else
            frappe.throw(
              "Postgrid Integration is not enabled. Enable it in Postgrid Configuration DocType."
            );
        });
    });
  },
