...
class BulkLetterCreationTool(Document):
    ...
    def get_sales_invoice(self):
        ...
        if self.from_date and self.to_date:
            condition += "and posting_date BETWEEN '{0}' and '{1}'".format(
        ...
        if si_list := frappe.db.sql(
            """Select name as sales_invoice,outstanding_amount as amount,status, customer, custom_postgrid_letter_reference from `tabSales Invoice`\
