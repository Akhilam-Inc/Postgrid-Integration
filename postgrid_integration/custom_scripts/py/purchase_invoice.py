...
def set_address(doc):
    if not doc.supplier_address:
        if supplier_address := frappe.db.sql(
            f""" Select a.name from `tabAddress` as a JOIN `tabDynamic Link` as dl ON dl.parent=a.name
													WHERE dl.link_doctype='Supplier' and dl.link_name='{doc.supplier}' and a.disabled=0 """,
            as_dict=1,
        ):
            doc.db_set(
                "supplier_address", supplier_address[0]["name"], update_modified=False
            )
            doc.db_set(
                "address_display",
                get_address_display(
                    frappe.get_doc("Address", supplier_address[0]["name"]).as_dict()
                ),
    ...
    if not doc.billing_address:
        if company_address := frappe.db.sql(
            ...
            doc.db_set(
                "billing_address", company_address[0]["name"], update_modified=False
            )
            doc.db_set(
...
def create_bulk_payment(invoice_list):
    ...
    for row in invoice_list:
        invoice_details = frappe.get_all(
            ...
            {"name": row},
            [
                "custom_postgrid_cheque_reference",
                "docstatus",
                "status",
                "outstanding_amount",
            ],
        ...
        if (
            not invoice_details.custom_postgrid_cheque_reference
            and invoice_details.docstatus == 1
            and invoice_details.outstanding_amount > 0
        ):
            bulk_payment_doc.append(
                "items",
                {
                    "purchase_invoice": row,
    ...
    if not bulk_payment_doc.items:
        frappe.throw(
            "The chosen records didn't meet the necessary criteria to generate postgrid cheques."
        )
