...
def check_print_format_mapped(doctype):
    ...
    if letter_format := frappe.get_all(
        "Postgrid Letter Format",
        {"doctype_name": doctype, "parent": "Postgrid Configuration"},
        ["format", "letterhead"],
    ):
        mapped = True
        ...
        frappe._dict(
            {
                ...
                "postgrid_cheque_reference"
                if "cheque" in args.url
                else "postgrid_letter_reference": data.id
                if response.ok and not webhook
                else "",
                "postgrid_cheque_status"
                if "cheque" in args.url
                else "postgrid_letter_status": data.status
                if response.ok and not webhook
                else "",
...
def get_pdf_link(doc, fieldname, url):
    print_format_details = frappe.get_all(
        "Postgrid Letter Format",
        {"doctype_name": doc.doctype, "parent": "Postgrid Configuration"},
        ["format", "letterhead"],
    )
    ...
    if not doc.get(fieldname):
        file_doc = upload_file(
            doctype=doc.doctype,
            name=doc.name,
            print_format=print_format_details[0]["format"],
            letterhead=print_format_details[0]["letterhead"],
            fieldname=fieldname,
        )
